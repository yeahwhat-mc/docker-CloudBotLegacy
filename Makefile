...
run: stop remove
	...
	@docker run -d --name="$(NAME)" -v $(shell pwd)/storage/data:/opt/cloudbot/data -v $(shell pwd)/storage/persist:/opt/cloudbot/persist $(AUTHOR)/$(NAME):$(VERSION) &> /dev/null && printf " done!\n" || printf " failed!\n"

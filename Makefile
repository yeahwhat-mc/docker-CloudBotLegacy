...
run: stop remove
	...
	@docker run -d --name="$(NAME)" -v $(shell pwd)/storage/data:/opt/cloudbot/data -v $(shell pwd)/storage/persist:/opt/cloudbot/persist -v $(shell pwd)/storage/plugins:/opt/cloudbot/plugins $(AUTHOR)/$(NAME):$(VERSION) &> /dev/null && printf " done!\n" || printf " failed!\n"
